...
def main() -> int:
    ...
    for cat, c in out.items():
        ...
        if plen > 0:
            ...
            out_text += markdown_table(sorted(out_list, key=lambda d: d['Name'].lower())).set_params(quote=False,padding_weight={'Name': 'right', 'linux-glibc-x86_64': 'centerright' , 'darwin-x86_64': 'centerright', 'darwin-aarch64': 'centerright'}).get_markdown()+'\n'
        else:
            out_text += '\n'
